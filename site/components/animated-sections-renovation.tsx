...
const renovationSections: SectionData[] = [
  {
    text: "Transformation Complète de votre Habitat",
    img: "/renovation.png"
  },
  {
    text: "Modernisation et Mise aux Normes",
    img: "/villamoderne.png"
  },
  {
    text: "Valorisation de votre Patrimoine",
    img: "/villa.png"
  }
];

const AnimatedSectionsRenovation: React.FC = () => {
  ...
  const hasSeenAllRef = useRef<boolean>(false);
  const sections = renovationSections;
  ...
  const gotoSection = useCallback((index: number, direction: number) => {
    ...
    // Track if user has seen all sections
    ...
    observerRef.current = Observer.create({
      ...
      onDown: () => {
        if (!animatingRef.current) {
          // If at first section and trying to scroll up, allow normal page scroll
          if (currentIndexRef.current === 0) {
      ...
      onUp: () => {
        if (!animatingRef.current) {
          // If user has seen all sections and is at the last one, allow scroll to continue
          if (hasSeenAllRef.current && currentIndexRef.current === sections.length - 1) {
            observerRef.current.disable();
            window.scrollBy({ top: 100, behavior: 'smooth' });
            setTimeout(() => {
              if (observerRef.current) {
...
export default AnimatedSectionsRenovation;